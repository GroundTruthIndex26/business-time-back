...
export function LaunchSignupDialog({ children, className, source, open: controlledOpen, onOpenChange }: { children?: ReactNode; className?: string; source: WaitlistSource; open?: boolean; onOpenChange?: (open: boolean) => void }) {
  ...
  const successIsNew = message === "You’re on the list. We’ll let you know when Business Time Back launches.";
  ...
  function setDialogOpen(nextOpen: boolean) {
    ...
    if (!nextOpen) { setMessage(null); setEmail(""); setValidationError(null); setIsSubmitting(false); }
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result?.error || "We couldn’t save your request. Please try again shortly.");
      setMessage(result?.isNew === false ? "You’re already on the list. We’ll be in touch at launch." : "You’re on the list. We’ll let you know when Business Time Back launches.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "We couldn’t save your request. Please try again shortly.");
    } finally { setIsSubmitting(false); }
  }

  return <>
    {children && <button className={className} type="button" onClick={() => setDialogOpen(true)}>{children}</button>}
    {open && <div className="waitlist-overlay" role="presentation" onMouseDown={() => setDialogOpen(false)}>
      <section className="waitlist-dialog" role="dialog" aria-modal="true" aria-labelledby={`${inputId}-title`} aria-describedby={`${inputId}-description`} onMouseDown={(event) => event.stopPropagation()}>
        <button className="waitlist-dialog__close" type="button" onClick={() => setDialogOpen(false)} aria-label="Close"><X size={18} /></button>
        <div className="waitlist-dialog__header">
          <span className="waitlist-dialog__eyebrow"><Mail size={14} /> Launch notification</span>
          <h2 id={`${inputId}-title`}>Be first to know when better work is visible.</h2>
          <p id={`${inputId}-description`}>Leave your email and we’ll let you know when Business Time Back is ready to explore.</p>
        </div>
        {message && !isSubmitting && !message.startsWith("We couldn’t") ? <div className="waitlist-dialog__success" role="status" aria-live="polite">
          <div className="waitlist-success__mark" aria-hidden="true"><span><Check size={27} strokeWidth={2.5} /></span><i /><b /></div>
          ...
          <p className="waitlist-dialog__privacy">We will use this email only for launch and relevant Business Time Back updates. No activity tracking. <a href={`${APP_ROOT}privacy`}>Privacy policy</a></p>
        </form>}
      </section>
    </div>}
  </>;
